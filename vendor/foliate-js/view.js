...
export const makeBook = async file => {
    ...
    if (file.isDirectory) {
        ...
        // [aireader 本地修改 L1] PDF 解析链路已打通（能识别、能打开、走 FOLIATE-FXL 渲染器），
        // 但画布始终是 0×0 —— 渲染没跑起来，原因待查。与其给用户一个空白页，不如明确拒绝。
        const { makePDF } = await import('./pdf.js')
        void makePDF
        throw new UnsupportedTypeError('PDF 渲染尚未完成（解析已打通，渲染待实现），当前版本请先转换为 EPUB')
    }
    else {
