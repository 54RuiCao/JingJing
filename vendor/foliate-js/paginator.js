...
export class Paginator extends HTMLElement {
    ...
    async #turnPage(dir, distance) {
        ...
        // [aireader 本地修改 L2] 上游在此硬编码 100ms 的翻页锁。
        // 实测：翻页的真实渲染耗时约 8ms（同章节内）/ 约 40ms（跨章节），
        // 也就是 100ms 中的绝大部分是这段人工延迟。改为可配置，默认仍为 100 以保持上游行为。
        if (shouldGo || !this.hasAttribute('animated'))
            await wait(Number(this.getAttribute('turn-lock') ?? 100))
        this.#locked = false
    }
    prev(distance) {
