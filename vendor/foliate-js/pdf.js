// [aireader 本地修改 L3] worker / cmaps / standard_fonts 这类**运行时静态资源**改为放在
// 前端根目录的 pdfjs/ 下（app/public/pdfjs，构建时原样拷进 dist），不走打包器的 URL 解析。
// 原因有两个：
//   1. 原写法缺少 `./` 前缀，Vite 直接报 [vite:import-glob] Invalid glob；
//   2. 补上前缀后，`new URL('./vendor/pdfjs/' + x, import.meta.url)` 对**目录**（cmaps/、
//      standard_fonts/）解析不出来，运行时报 Invalid factory url: ".../assets/undefined"。
