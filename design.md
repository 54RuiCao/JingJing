...
## 9. 阅读页浮层规范（照 453 参考，P11）

阅读页是"一页纸"优先：**控件默认全部隐藏**，点屏幕中间一次，顶栏与底栏一起淡入，
4 秒后自动收起（再点一次也收起）。
...
- **不放书名**（会被按钮挤到重叠）、不放百分比（页码在底栏）
- 44×44 的图标位、强调色、无边框；整条高 48px + 安全区
- 绝对定位**浮在正文之上**（不占文档流），毛玻璃 + 底部发丝线
- 正文区顶部留 `max(env(safe-area-inset-top), 26px)`，让浮栏压在阅读引擎自己的 48px 页边上

### 9.2 底栏：只有"页码 + AI"

`[位置 23 · 2.3%]                    [ 🤖 AI ]`

- 左侧页码：`13.5px`、`--m-sub`、等宽数字、单行省略
- 右侧 AI：药丸、强调色底、白字、40px 高
- 整条高 55px + 安全区，毛玻璃；插件贡献的"阅读区尾部"抬到它上方（`bottom: 56px`）

### 9.3 面板（卡片）里的滚动

**一张卡片里只能有一个滚动条。** 我们的做法（半屏 AI 卡片）：
卡片里**整块对话区**滚（`.air-chat`），消息列表不再自己滚，输入条 `position: sticky; bottom: 0` 吸附在底部。
再加 `overscroll-behavior: contain` —— 否则手指一划，滚的是**背后的正文**（用户实测的坑）。

> 踩过的坑：把设置块做成 `position: fixed` 的独立卡片 —— 祖先有 `backdrop-filter` + `overflow: hidden` 时，
> fixed 的包含块变成那张卡片，顶部连同标题被裁掉。**别在玻璃卡片里用 fixed 子树**。

### 9.4 输入条

- 高 **44px** 起步，随输入长到最多 4 行（96px）后内部滚动
- 必须写 `box-sizing: border-box` —— 项目没有全局的，`height: 44px + padding` 会变成 66px
- 占位符在手机上要短（"问点什么…（/ 调用技能）"），长占位符会被截断
- 毛玻璃底 + 顶部发丝线，发送按钮是强调色药丸

### 9.5 AI 设置面板

一行一个字段：**标签在上（13.5px 灰、半粗）→ 控件满宽（44px 高、圆角 10px）→ 说明小一号灰字**；
勾选项做成"一行 44px、方块 + 文字"；标题行（"设置" + ✕）用一条发丝线与内容分开。
不要虚线框、不要内联 style 的杂色。
...
用户四轮反馈收敛后的定稿：**顶部一排图标、底部一个输入框、页码在正文右下角**。

### 10.1 顶栏

`← (左)  ·····  ☰ Aa 🔍 🔖 (右)`

- **只有返回键在左**，其余图标全部靠右（实现：第一个按钮 `margin-right: auto`）
- 44×44 图标位、强调色、无边框；48px 高 + 安全区；绝对定位浮在正文上（毛玻璃 + 发丝线）
- 不放书名、不放百分比

### 10.2 底栏：**只有一条**，内容是一个输入框

`[ 插件尾部状态（可选，一行） ]`
`[ 🤖 问点什么…（/ 调用技能） ]`

- 插件贡献的"阅读区尾部"**搬进这条栏里面**（原来是两条：插件那条不透明、底栏那条透明，叠在一起很难看）
- AI 入口做成**输入框的样子**（药丸、`--m-panel 72%`、发丝边、左侧 AI 图标用强调色）——
  不需要先找到"AI 按钮"：点屏幕中间 → 底栏出来 → 点输入框 → 半透明 AI 卡片升起
- 高度：输入框 44px；整条栏 = 尾部一行 + 8px 间距 + 44px + 安全区
- 尾部那一行长文本要 `nowrap + ellipsis`，里面的插件按钮压到 28px 高、`font-size: 12px`
  （否则真机上"回开头"会被折成两行）

### 10.3 页码：正文右下角，常驻

- `position: absolute; right: 14px; bottom: 10px + 安全区`，12px、`--m-sub`、等宽数字、`pointer-events: none`
- 底栏出现时**抬到它上面**（`bottom: 70px + 安全区`），别被压住

### 10.4 AI 卡片里的输入条

- **贴卡片底部**：`margin-top: auto`（内容不满一屏时也不许悬在半空 —— 用户截图里的问题）
- 输入框 40px、发送按钮 40px、发丝线分隔、毛玻璃底
- 手机占位符要短（"问点什么…（/ 调用技能）"）


---

## 11. P13 收尾规则（用户逐条反馈后的定稿）

1. **页码用"当前/总数"**（如 `23/1008`），不要"位置 23"。数据源是 foliate relocate 事件里的
   `location.current / location.total`，拿不到时才回退到位置文本或百分比。
2. **底栏里不放插件状态条**。插件的 `reader.view.tail` 在手机端不渲染（桌面照旧）——
   底栏只留一个输入框；状态信息统一由右下角页码承担，避免同一屏两处说同一件事。
3. **没有功能的东西不要摆在界面上**。例如页头右上角那个"共 N 本"：它长得像筛选标签却点不动，
   手机端直接不显示（计数在列表底部居中那行已经有了）。
4. **一切都给圆角**：输入框/下拉 10px、按钮与胶囊 999px、卡片 12px、底部卡片 18px、
   抽屉页签 10px。项目里没有全局 `box-sizing`，写 `height` 时记得配 `border-box`。
5. **抽屉页签 = 等宽分段控件**：`flex: 1 1 0`、统一 40px 高、轨道 `--m-grouped` + 10px 圆角、
   选中项白底 + 轻投影。标签在手机端用**短词**（目录/批注/检索/设置），否则宽度会被文字撑得参差。
6. **AI（整页与半屏）统一**：底色 `--m-glass` + `saturate(180%) blur(24px)`；
   内部的上下文条、技能条、设置块、AI 气泡都用 `--m-panel 55~62%` 的半透明底 + 发丝边；
   输入条用 `--m-glass-strong`。整块看起来是一张玻璃，而不是几个白盒子。


---

## 12. AI 页面（照 ChatGPT iOS 23，P20）

- **输入条 = 悬浮胶囊**：左右各 `--m-pad`、高 46px、圆角 999px、毛玻璃（面板色 72% + `saturate(180%) blur(24px)`）+ 外投影 + 发丝边。
- **发送 = 圆形实心钮**（46×46，强调色底、白字），不是长条药丸。
- 输入条**贴在底部栏上方**（页面内容底部留 `74px + max(安全区,10px)`，正好是胶囊高度 + 呼吸），中间不留大片空白。
- 顶部**不放关闭按钮**：整页 AI 的出口就是底部页签；阅读页那张卡片连头部一起没有，靠点遮罩或系统返回键关。
- 建议卡（参考里输入框上方那两张小卡）：**暂未做**，现在仍是一行行示例文字。

## 13. 底部任务栏（照 Apple Books iOS 7，P20）

- 浮动胶囊**水平居中**（`justify-content: center`，最小宽 250px），不与屏幕左右边缘相接（左右各 12px）。
- 距底：`10px + max(env(safe-area-inset-bottom), 10px)` —— **必须有那个 max 兜底**：
  沉浸式下 `env(safe-area-inset-bottom)` 可能是 0，实测会出现"任务栏被屏幕切掉一半"。
- 选中格：浅底圆角块 + 强调色图标/文字；未选中只有灰图标与 10.5px 灰字。
